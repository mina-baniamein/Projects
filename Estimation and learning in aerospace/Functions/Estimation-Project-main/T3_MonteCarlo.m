...
for i = 1:N
    ...
    [num, den] = tfdata(tf(sys_i), 'v');
    zk = roots(num);
    zeros_grey = [zeros_grey; zk(:)];
end
...
semilogx(w, (mean_phase + 5* std_phase)*180/pi, 'r--');
semilogx(w, (mean_phase - 5* std_phase)*180/pi, 'r--');
xlabel('Frequency [rad/s]');
ylabel('Phase [deg]');
title('Grey-Box FRF Phase Dispersion');
grid on;
legend('Mean value', 'mu+1σ', 'mu-1σ');
...
nexttile; density_complex(poles_grey);  title('Poles Density - Grey Box'); xlabel('Re'); ylabel('Im'); 
nexttile; density_complex(zeros_grey);  title('Zeros Density - Grey Box'); xlabel('Re'); ylabel('Im'); 

plot_pz_histograms(poles_grey, zeros_grey)

figure;
subplot(1,2,1);
histogram(real(poles_grey(:, 1)), 30);
xlabel('Re(λ)');
ylabel('Count');
title('Grey-Box: First Pole Real Part');
grid on;

subplot(1,2,2);
histogram(imag(poles_grey(:, 1)), 30);
xlabel('Im(λ)');
ylabel('Count');
title('Grey-Box: First Pole Imaginary Part');
grid on;
...
nexttile; density_complex(poles_black);  title('Poles Density - PBSID'); xlabel('Re'); ylabel('Im'); 
nexttile; density_complex(zeros_black);  title('Zeros Density - PBSID'); xlabel('Re'); ylabel('Im'); 

plot_pz_histograms(poles_black, zeros_black)

figure;
subplot(1,2,1);
histogram(real(poles_black(:, 1)), 30);
xlabel('Re(λ)');
ylabel('Count');
...
subplot(1,2,2);
histogram(imag(poles_black(:, 1)), 30);
xlabel('Im(λ)');
ylabel('Count');
