function [mc_results, mc_summary] = monte_carlo_structuring(u_3ord, y_q, t_3ord, ...
    Xu, Xq, Mu, Mq, Xd, Md, sample_time, fc, p_value, n_order, varargin)
...
addRequired(p, 't_3ord', @isnumeric);
...
parse(p, u_3ord, y_q, t_3ord, Xu, Xq, Mu, Mq, Xd, Md, sample_time, fc, p_value, n_order, varargin{:});
...
for mc_run = 1:num_monte_carlo
    ...
    input_noise = noise.ang_rate_stand_dev * 0.1 * randn(size(u_3ord));
    ...
    try
        ...
        u_filt = filtfilt(b, a, u_3ord_noisy);
        y_filt = filtfilt(b, a, y_3ord_noisy);
