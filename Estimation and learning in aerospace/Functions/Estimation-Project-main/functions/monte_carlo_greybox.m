function [mc_results, mc_summary] = monte_carlo_greybox(u_3ord, y_q, y_ax, ...
...
%% Input Parsing
p = inputParser;
addRequired(p, 'u_3ord', @isnumeric);
addRequired(p, 'y_q', @isnumeric);
addRequired(p, 'y_ax', @isnumeric);
...
parse(p, u_3ord, y_q, y_ax, Xu, Xq, Mu, Mq, Xd, Md, sample_time, varargin{:});
...
for mc_run = 1:num_monte_carlo
    ...
    %% Generate Measurement Noise Realizations
    % Angular rate measurement noise (affects y_q - pitch rate)
    ang_rate_noise_q = noise.ang_rate_stand_dev * randn(size(y_q));
    y_q_noisy = y_q + ang_rate_noise_q;
    ...
    input_noise = noise.ang_rate_stand_dev * 0.1 * randn(size(u_3ord));  % Reduced factor for input
    u_3ord_noisy = u_3ord + input_noise;
    
    %% Construct Noisy Output Vector
    % Combined noisy output vector (same structure as original)
    y_grey_noisy = [y_q_noisy y_ax_noisy];
    ...
    try
        [identification_grey, error_grey] = Model_identification(u_3ord_noisy, y_grey_noisy, ...
